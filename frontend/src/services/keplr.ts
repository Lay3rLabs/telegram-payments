 ...
 */
export async function suggestChain(chainId: string, chainName: string, rpcEndpoint: string): Promise<void> {
  if (!window.keplr) {
  ...
  await window.keplr.experimentalSuggestChain({
    ...
    bech32Config: {
      ...
      bech32PrefixAccPub: chainId === 'cosmoshub-4' ? 'cosmospub' : `${chainId}pub`,
      bech32PrefixValAddr: chainId === 'cosmoshub-4' ? 'cosmosvaloper' : `${chainId}valoper`,
      bech32PrefixValPub: chainId === 'cosmoshub-4' ? 'cosmosvaloperpub' : `${chainId}valoperpub`,
      bech32PrefixConsAddr: chainId === 'cosmoshub-4' ? 'cosmosvalcons' : `${chainId}valcons`,
      bech32PrefixConsPub: chainId === 'cosmoshub-4' ? 'cosmosvalconspub' : `${chainId}valconspub`,
    },
    currencies: [
      {
        coinDenom: 'ATOM',
 ...
 */
export async function connectKeplr(chainId: string = 'cosmoshub-4'): Promise<KeplrAccount> {
  if (!window.keplr) {
    throw new Error('Keplr extension not installed. Please install Keplr from https://www.keplr.app/');
  }
 ...
 */
export async function getKeplrAccount(chainId: string = 'cosmoshub-4'): Promise<KeplrAccount | null> {
  if (!window.keplr) {
    return null;
  }
