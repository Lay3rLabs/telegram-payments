...
pub struct TelegramMessenger<C: HttpClient> {
    pub token: String,
    pub client: C,
}

impl<C: HttpClient> TelegramMessenger<C> {
    pub fn new(token: String, client: C) -> Self {
        Self { token, client }
    }
    ...
    async fn make_request_empty<T: DeserializeOwned>(&self, method: &str) -> TgResult<T> {
        ...
        let text = self.client.get(&url).await?;
    ...
    async fn make_request_params<T: DeserializeOwned>(
        ...
        let text = self.client.post_form(&url, params).await?;
